...
class PostGISSearch:
    ...
    def cellClicked(self):
        ...
        for column in range(hmodel.columnCount()):
            if hmodel.headerData(column, Qt.Horizontal) == "geom":
                geomcolumn = column

            elif hmodel.headerData(column, Qt.Horizontal) == "x":
                xcolumn = column

            elif hmodel.headerData(column, Qt.Horizontal) == "y":
                ycolumn = column

            elif hmodel.headerData(column, Qt.Horizontal) == self.postgiscolumn:
                descriptioncolumn = column

        geom = self.projectModel.index(rownumber, geomcolumn).data()
